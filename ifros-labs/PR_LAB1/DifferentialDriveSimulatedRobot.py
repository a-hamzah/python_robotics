...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[], *args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        # simulated acceleration noise
        self.Qsk = np.diag(np.array([0.1 ** 2, 0.01 ** 2, np.deg2rad(1) ** 2]))
        ...
        # covariance of simulated wheel encoder noise
        self.Re = np.diag(np.array([22 ** 2, 22 ** 2]))

        # frequency of Polar2D feature readings
        self.Polar2D_feature_reading_frequency = 50
        # maximum Polar2D range, used to simulate the field of view
        self.Polar2D_max_range = 50
        # covariance of simulated Polar2D feature noise
        self.Rfp = np.diag(np.array([1 ** 2, np.deg2rad(5) ** 2]))
        ...
        # std deviation of simulated heading noise
        self.v_yaw_std = np.deg2rad(5)
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        #
        etask_1 = Pose3D(xsk_1[:3])  # previous pose
        nusk_1 = Pose3D(xsk_1[3:])  # previous velocity
        nu_d = np.array([[usk[0, 0]], [0.0], [usk[1, 0]]])  # desired velocity

        # sample Gaussian for the acceleration perturbation
        wsk = np.reshape(np.random.multivariate_normal([0.0, 0.0, 0.0], self.Qsk), (3, 1))
